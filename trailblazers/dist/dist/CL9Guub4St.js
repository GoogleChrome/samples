...
(async function () {
  ...
  // ── Initialise AI ────────────────────────────────────────────────────────────
  try {
    const { getAvailability, initClassifier, classifyComment } =
      await import('/js/ai/ai-comment-classifier.js');
    ...
    if (availability === 'unavailable') {
      ...
      const ok = await initClassifier((loaded, total) => {
      ...
      aiReady = ok;
      if (!ok) showUnavailableNotice();
    } else {
      // available — warm the session in the background
      initClassifier().then((ok) => {
        aiReady = ok;
        if (!ok) showUnavailableNotice();
      });
      aiReady = true; // optimistic: session will be ready by submit time
    }
    ...
    form.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        statusEl.hidden = true;
        // Graceful fallback: post without review
        appendComment(name, text);
      } finally {
    ...
    console.error('[comments] AI module failed to load:', err);
    showUnavailableNotice();
  ...
  function buildCommentElement(name, text) {
    ...
    article.innerHTML = `
      <div class="comment-header">
        ...
        <span class="comment-badge-safe"></span>
      </div>
      <p class="comment-text"></p>
    `;

    // Set user-provided data via textContent to prevent XSS
    ...
    article.querySelector('.comment-date').textContent   = now;
    article.querySelector('.comment-badge-safe').textContent = `✓ ${labels.safeBadge}`;
