import { FirebaseError as e, isCloudWorkstation as t, base64 as r, DecodeBase64StringError as n, getDefaultEmulatorHostnameAndPort as i, pingServer as s, deepEqual as o, createMockUserToken as a, getModularInstance as u } from "@firebase/util";

import { Logger as _, LogLevel as c } from "@firebase/logger";

import { Integer as l } from "@firebase/webchannel-wrapper/bloom-blob";

import { _isFirebaseServerApp as h, _getProvider, getApp as f, _removeServiceInstance as d } from "@firebase/app";
 ...
 * limitations under the License.
 */
const E = new _("@firebase/firestore");
...
function __PRIVATE_logDebug(e, ...t) {
    if (E.logLevel <= c.DEBUG) {
        const r = t.map(__PRIVATE_argToString);
        E.debug(`Firestore (${m}): ${e}`, ...r);
    }
}

function __PRIVATE_logError(e, ...t) {
    if (E.logLevel <= c.ERROR) {
        const r = t.map(__PRIVATE_argToString);
        E.error(`Firestore (${m}): ${e}`, ...r);
    }
}
 ...
 */ function __PRIVATE_logWarn(e, ...t) {
    if (E.logLevel <= c.WARN) {
        const r = t.map(__PRIVATE_argToString);
        E.warn(`Firestore (${m}): ${e}`, ...r);
    }
}
...
function __PRIVATE__fail(e, t, r) {
    ...
    let n = `FIRESTORE (${m}) INTERNAL ASSERTION FAILED: ${t} (ID: ${e.toString(16)})`;
    if (void 0 !== r) try {
...
/** AppCheck token provider for the Lite SDK. */ class __PRIVATE_LiteAppCheckTokenProvider {
    constructor(e, t) {
        this.m = t, this.appCheck = null, this.P = null, h(e) && e.settings.appCheckToken && (this.P = e.settings.appCheckToken), 
        t.onInit((e => {
            this.appCheck = e;
        }));
    }
...
/** The default database name for a project. */ const T = "(default)";
 ...
 */ class DatabaseId {
    constructor(e, t) {
        this.projectId = e, this.database = t || T;
    }
    static empty() {
        return new DatabaseId("", "");
    }
    get isDefaultDatabase() {
        return this.database === T;
    }
    isEqual(e) {
...
function __PRIVATE_databaseIdFromApp(e, t) {
    if (!Object.prototype.hasOwnProperty.apply(e.options, [ "projectId" ])) throw new FirestoreError(P.INVALID_ARGUMENT, '"projectId" not provided in firebase.initializeApp.');
 ...
 */ class BasePath {
    ...
    static extractNumericId(e) {
        return l.fromString(e.substring(4, e.length - 2));
    }
}
 ...
 */ class ResourcePath extends BasePath {
     ...
     */    static fromString(...e) {
        ...
        const t = [];
        for (const r of e) {
            if (r.indexOf("//") >= 0) throw new FirestoreError(P.INVALID_ARGUMENT, `Invalid segment (${r}). Paths must not contain // in them.`);
 ...
 */ class FieldPath$1 extends BasePath {
     ...
     */    isKeyField() {
        return 1 === this.length && this.get(0) === A;
    }
    /**
     * The field designating the key of a document.
     */    static keyField() {
        return new FieldPath$1([ A ]);
    }
    /**
     ...
     */    static fromServerFormat(e) {
        ...
        const __PRIVATE_addCurrentSegment = () => {
            if (0 === r.length) throw new FirestoreError(P.INVALID_ARGUMENT, `Invalid field path (${e}). Paths must not be empty, begin with '.', end with '.', or contain '..'`);
            t.push(r), r = "";
        };
        let i = !1;
        for (;n < e.length; ) {
            const t = e[n];
            if ("\\" === t) {
                if (n + 1 === e.length) throw new FirestoreError(P.INVALID_ARGUMENT, "Path has trailing escape character: " + e);
                const t = e[n + 1];
                if ("\\" !== t && "." !== t && "`" !== t) throw new FirestoreError(P.INVALID_ARGUMENT, "Path has invalid escape sequence: " + e);
                r += t, n += 2;
            } else "`" === t ? (i = !i, n++) : "." !== t || i ? (r += t, n++) : (__PRIVATE_addCurrentSegment(), 
            n++);
        }
        if (__PRIVATE_addCurrentSegment(), i) throw new FirestoreError(P.INVALID_ARGUMENT, "Unterminated ` in path: " + e);
 ...
 */ function __PRIVATE_validateNonEmptyArgument(e, t, r) {
    if (!r) throw new FirestoreError(P.INVALID_ARGUMENT, `Function ${e}() cannot be called with an empty ${t}.`);
}
...
function __PRIVATE_validateDocumentPath(e) {
    if (!DocumentKey.isDocumentKey(e)) throw new FirestoreError(P.INVALID_ARGUMENT, `Invalid document reference. Document references must have an even number of segments, but ${e} has ${e.length}.`);
}
 ...
 */ function __PRIVATE_validateCollectionPath(e) {
    if (DocumentKey.isDocumentKey(e)) throw new FirestoreError(P.INVALID_ARGUMENT, `Invalid collection reference. Collection references must have an odd number of segments, but ${e} has ${e.length}.`);
}
...
t) {
    ...
    e = e._delegate), !(e instanceof t)) {
        if (t.name === e.constructor.name) throw new FirestoreError(P.INVALID_ARGUMENT, "Type does not match the expected instance. Did you pass a reference from a different Firestore SDK?");
        {
            const r = __PRIVATE_valueDescription(e);
            throw new FirestoreError(P.INVALID_ARGUMENT, `Expected type '${t.name}', but it was: ${r}`);
        }
    }
    return e;
}
...
function __PRIVATE_validatePositiveNumber(e, t) {
    if (t <= 0) throw new FirestoreError(P.INVALID_ARGUMENT, `Function ${e}() requires a positive number, but it was: ${t}.`);
}
...
function __PRIVATE_generateUniqueDebugId() {
    return null === p ? p = function __PRIVATE_generateInitialUniqueDebugId() {
...
class __PRIVATE_RestConnection {
    ...
    constructor(e) {
        ...
        this.R = t + "://" + e.host, this.V = `projects/${r}/databases/${n}`, this.A = this.databaseId.database === T ? `project_id=${r}` : `project_id=${r}&database_id=${n}`;
    }
    I(e, r, n, i, s) {
        const o = __PRIVATE_generateUniqueDebugId(), a = this.p(e, r.toUriEncodedString());
        __PRIVATE_logDebug(y, `Sending RPC '${e}' ${o}:`, a, n);
        const u = {
        ...
        return this.v(e, a, u, n, c).then((t => (__PRIVATE_logDebug(y, `Received RPC '${e}' ${o}: `, t), 
        t)), (t => {
            throw __PRIVATE_logWarn(y, `RPC '${e}' ${o} failed with error: `, t, "url: ", a, "request:", n), 
            t;
        }));
    }
    D(e, t, r, n, i, s) {
 ...
 */ function __PRIVATE_mapCodeFromHttpStatus(e) {
    ...
    // https://github.com/googleapis/googleapis/blob/master/google/rpc/code.proto
      ...
      default:
        return e >= 200 && e < 300 ? P.OK : e >= 400 && e < 500 ? P.FAILED_PRECONDITION : e >= 500 && e < 600 ? P.INTERNAL : P.UNKNOWN;
    }
}
 ...
 */ (F = g || (g = {}))[F.OK = 0] = "OK", F[F.CANCELLED = 1] = "CANCELLED", F[F.UNKNOWN = 2] = "UNKNOWN", 
F[F.INVALID_ARGUMENT = 3] = "INVALID_ARGUMENT", F[F.DEADLINE_EXCEEDED = 4] = "DEADLINE_EXCEEDED", 
F[F.NOT_FOUND = 5] = "NOT_FOUND", F[F.ALREADY_EXISTS = 6] = "ALREADY_EXISTS", F[F.PERMISSION_DENIED = 7] = "PERMISSION_DENIED", 
F[F.UNAUTHENTICATED = 16] = "UNAUTHENTICATED", F[F.RESOURCE_EXHAUSTED = 8] = "RESOURCE_EXHAUSTED", 
F[F.FAILED_PRECONDITION = 9] = "FAILED_PRECONDITION", F[F.ABORTED = 10] = "ABORTED", 
F[F.OUT_OF_RANGE = 11] = "OUT_OF_RANGE", F[F.UNIMPLEMENTED = 12] = "UNIMPLEMENTED", 
F[F.INTERNAL = 13] = "INTERNAL", F[F.UNAVAILABLE = 14] = "UNAVAILABLE", F[F.DATA_LOSS = 15] = "DATA_LOSS";
...
const b = new RegExp(/^\d{4}-\d\d-\d\dT\d\d:\d\d:\d\d(?:\.(\d+))?Z$/);
 ...
 */ function __PRIVATE_validateJSON(e, t) {
    if (!__PRIVATE_isPlainObject(e)) throw new FirestoreError(P.INVALID_ARGUMENT, "JSON must be an object");
    let r;
    ...
    if (r) throw new FirestoreError(P.INVALID_ARGUMENT, r);
    return !0;
}
...
class Timestamp {
     ...
     */    static fromMillis(e) {
        const t = Math.floor(e / 1e3), r = Math.floor((e - 1e3 * t) * S);
     ...
     */
    t) {
        if (this.seconds = e, this.nanoseconds = t, t < 0) throw new FirestoreError(P.INVALID_ARGUMENT, "Timestamp nanoseconds out of range: " + t);
        if (t >= 1e9) throw new FirestoreError(P.INVALID_ARGUMENT, "Timestamp nanoseconds out of range: " + t);
        if (e < D) throw new FirestoreError(P.INVALID_ARGUMENT, "Timestamp seconds out of range: " + e);
        // This will break in the year 10,000.
                if (e >= 253402300800) throw new FirestoreError(P.INVALID_ARGUMENT, "Timestamp seconds out of range: " + e);
    }
    /**
     ...
     */    toMillis() {
        return 1e3 * this.seconds + this.nanoseconds / S;
    }
    _compareTo(e) {
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */ const C = "__type__", N = "__max__", O = {
    fields: {
        __type__: {
            stringValue: N
        }
    }
}, q = "__vector__", B = "value";
...
function __PRIVATE_typeOrder(e) {
    ...
    function __PRIVATE_isMaxValue(e) {
        return (((e.mapValue || {}).fields || {}).__type__ || {}).stringValue === N;
    }
    /**
 * @license
 ...
 */ (e) ? 9007199254740991 /* TypeOrder.MaxValue */ : 
    ...
    function __PRIVATE_isVectorValue(e) {
        const t = (e?.mapValue?.fields || {})[C]?.stringValue;
        return t === q;
    }
    ...
    switch (r) {
      ...
      case 10 /* TypeOrder.VectorValue */ :
        return function __PRIVATE_compareVectors(e, t) {
            const r = e.fields || {}, n = t.fields || {}, i = r[B]?.arrayValue, s = n[B]?.arrayValue, o = __PRIVATE_primitiveComparator(i?.values?.length || 0, s?.values?.length || 0);
            if (0 !== o) return o;
...
function fromName(e, t) {
    ...
    if (r.get(1) !== e.databaseId.projectId) throw new FirestoreError(P.INVALID_ARGUMENT, "Tried to deserialize key from different project: " + r.get(1) + " vs " + e.databaseId.projectId);
    if (r.get(3) !== e.databaseId.database) throw new FirestoreError(P.INVALID_ARGUMENT, "Tried to deserialize key from different database: " + r.get(3) + " vs " + e.databaseId.database);
 ...
 */ class __PRIVATE_DatastoreImpl extends Datastore {
    ...
    j() {
        if (this.k) throw new FirestoreError(P.FAILED_PRECONDITION, "The client has already been terminated.");
    }
    /** Invokes the provided RPC with auth and AppCheck tokens. */    I(e, t, r, n) {
        return this.j(), Promise.all([ this.authCredentials.getToken(), this.appCheckCredentials.getToken() ]).then((([i, s]) => this.connection.I(e, __PRIVATE_toResourcePath(t, r), n, i, s))).catch((e => {
            throw "FirebaseError" === e.name ? (e.code === P.UNAUTHENTICATED && (this.authCredentials.invalidateToken(), 
            this.appCheckCredentials.invalidateToken()), e) : new FirestoreError(P.UNKNOWN, e.toString());
        }));
    }
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */ const U = "ComponentProvider", M = new Map;
...
function __PRIVATE_getDatastore(e) {
    if (e._terminated) throw new FirestoreError(P.FAILED_PRECONDITION, "The client has already been terminated.");
    if (!M.has(e)) {
        __PRIVATE_logDebug(U, "Initializing Datastore");
 ...
 */ const k = 1048576, x = "firestore.googleapis.com", j = !0;
...
class FirestoreSettingsImpl {
    constructor(e) {
        if (void 0 === e.host) {
            if (void 0 !== e.ssl) throw new FirestoreError(P.INVALID_ARGUMENT, "Can't provide ssl option if host option is not set");
            this.host = x, this.ssl = j;
        } else this.host = e.host, this.ssl = e.ssl ?? j;
        ...
        void 0 === e.cacheSizeBytes) this.cacheSizeBytes = 41943040; else {
            if (-1 !== e.cacheSizeBytes && e.cacheSizeBytes < k) throw new FirestoreError(P.INVALID_ARGUMENT, "cacheSizeBytes must be at least 1048576");
            this.cacheSizeBytes = e.cacheSizeBytes;
        }
        !function __PRIVATE_validateIsNotUsedTogether(e, t, r, n) {
            if (!0 === t && !0 === n) throw new FirestoreError(P.INVALID_ARGUMENT, `${e} and ${r} cannot be used together.`);
        ...
        function __PRIVATE_validateLongPollingOptions(e) {
            if (void 0 !== e.timeoutSeconds) {
                if (isNaN(e.timeoutSeconds)) throw new FirestoreError(P.INVALID_ARGUMENT, `invalid long polling timeout: ${e.timeoutSeconds} (must not be NaN)`);
                if (e.timeoutSeconds < 5) throw new FirestoreError(P.INVALID_ARGUMENT, `invalid long polling timeout: ${e.timeoutSeconds} (minimum allowed value is 5)`);
                if (e.timeoutSeconds > 30) throw new FirestoreError(P.INVALID_ARGUMENT, `invalid long polling timeout: ${e.timeoutSeconds} (maximum allowed value is 30)`);
            }
        }
        /**
 * @license
...
class Firestore {
     ...
     * instance.
     */    get app() {
        if (!this._app) throw new FirestoreError(P.FAILED_PRECONDITION, "Firestore was not initialized using the Firebase SDK. 'app' is not available");
        return this._app;
    }
    get _initialized() {
    ...
    _setSettings(e) {
        if (this._settingsFrozen) throw new FirestoreError(P.FAILED_PRECONDITION, "Firestore has already been started and its settings can no longer be changed. You can only modify settings before calling any other methods on a Firestore object.");
        ...
        void 0 !== e.credentials && (this._authCredentials = function __PRIVATE_makeAuthCredentialsProvider(e) {
            ...
            switch (e.type) {
              ...
              default:
                throw new FirestoreError(P.INVALID_ARGUMENT, "makeAuthCredentialsProvider failed due to invalid credential type");
            }
        }(e.credentials));
    }
    _getSettings() {
     ...
     */    _terminate() {
        return function __PRIVATE_removeComponents(e) {
            const t = M.get(e);
            t && (__PRIVATE_logDebug(U, "Removing Datastore"), M.delete(e), t.terminate());
...
function initializeFirestore(e, t, r) {
    ...
    if (n.isInitialized(r)) throw new FirestoreError(P.FAILED_PRECONDITION, "Firestore can only be initialized once per app.");
    return n.initialize({
        options: t,
...
function getFirestore(e, t) {
    const r = "object" == typeof e ? e : f(), n = "string" == typeof e ? e : t || "(default)", s = _getProvider(r, "firestore/lite").getImmediate({
        identifier: n
    });
    if (!s._initialized) {
        const e = i("firestore");
 ...
 */ function connectFirestoreEmulator(e, r, n, i = {}) {
    ...
    const u = t(r), _ = e._getSettings(), c = {
        ..._,
        emulatorOptions: e._getEmulatorOptions()
    }, l = `${r}:${n}`;
    u && s(`https://${l}`), _.host !== x && _.host !== l && __PRIVATE_logWarn("Host has been set in both settings() and connectFirestoreEmulator(), emulator host will be used.");
    const h = {
        ..._,
        host: l,
        ssl: u,
    ...
    // enviornments which might call `connectFirestoreEmulator` multiple times as a standard practice.
        if (!o(h, c) && (e._setSettings(h), i.mockUserToken)) {
        let t, r;
        if ("string" == typeof i.mockUserToken) t = i.mockUserToken, r = User.MOCK_USER; else {
            // Let createMockUserToken validate first (catches common mistakes like
            // invalid field "uid" and missing field "sub" / "user_id".)
            t = a(i.mockUserToken, e._app?.options.projectId);
            const n = i.mockUserToken.sub || i.mockUserToken.user_id;
            if (!n) throw new FirestoreError(P.INVALID_ARGUMENT, "mockUserToken must contain 'sub' or 'user_id' field!");
            r = new User(n);
        }
 ...
 */ function terminate(e) {
    return e = __PRIVATE_cast(e, Firestore), d(e.app, "firestore/lite"), e._delete();
}
...
function collection(e, t, ...r) {
    if (e = u(e), __PRIVATE_validateNonEmptyArgument("collection", "path", t), e instanceof Firestore) {
        const n = ResourcePath.fromString(t, ...r);
        return __PRIVATE_validateCollectionPath(n), new CollectionReference(e, /* converter= */ null, n);
    }
    {
        if (!(e instanceof DocumentReference || e instanceof CollectionReference)) throw new FirestoreError(P.INVALID_ARGUMENT, "Expected first argument to collection() to be a CollectionReference, a DocumentReference or FirebaseFirestore");
 ...
 */ function collectionGroup(e, t) {
    ...
    t.indexOf("/") >= 0) throw new FirestoreError(P.INVALID_ARGUMENT, `Invalid collection ID '${t}' passed to function collectionGroup(). Collection IDs must not contain '/'.`);
    return new Query(e, 
...
function doc(e, t, ...r) {
    ...
    {
        if (!(e instanceof DocumentReference || e instanceof CollectionReference)) throw new FirestoreError(P.INVALID_ARGUMENT, "Expected first argument to doc() to be a CollectionReference, a DocumentReference or FirebaseFirestore");
        const n = e._path.child(ResourcePath.fromString(t, ...r));
 ...
 */ function refEqual(e, t) {
    return e = u(e), t = u(t), (e instanceof DocumentReference || e instanceof CollectionReference) && (t instanceof DocumentReference || t instanceof CollectionReference) && (e.firestore === t.firestore && e.path === t.path && e.converter === t.converter);
}
 ...
 */ function queryEqual(e, t) {
    return e = u(e), t = u(t), e instanceof Query && t instanceof Query && (e.firestore === t.firestore && __PRIVATE_queryEquals(e._query, t._query) && e.converter === t.converter);
}
 ...
 */ class Bytes {
     ...
     */    static fromBase64String(e) {
        try {
            ...
            throw new FirestoreError(P.INVALID_ARGUMENT, "Failed to construct data from Base64 string: " + e);
        }
    }
    /**
...
class FieldPath {
     ...
     */
    constructor(...e) {
        for (let t = 0; t < e.length; ++t) if (0 === e[t].length) throw new FirestoreError(P.INVALID_ARGUMENT, "Invalid field name at argument $(i + 1). Field names must not be empty.");
 ...
 */ class GeoPoint {
     ...
     */
    constructor(e, t) {
        if (!isFinite(e) || e < -90 || e > 90) throw new FirestoreError(P.INVALID_ARGUMENT, "Latitude must be a number between -90 and 90, but was: " + e);
        if (!isFinite(t) || t < -180 || t > 180) throw new FirestoreError(P.INVALID_ARGUMENT, "Longitude must be a number between -180 and 180, but was: " + t);
 ...
 */
     ...
     */    static fromJSON(e) {
        if (__PRIVATE_validateJSON(e, VectorValue._jsonSchema)) {
            ...
            throw new FirestoreError(P.INVALID_ARGUMENT, "Expected 'vectorValues' field to be a number array");
        }
    }
}
...
/** A "context" object passed around while parsing user data. */ class __PRIVATE_ParseContextImpl {
    ...
    H(e) {
        ...
        if (__PRIVATE_isWrite(this.dataSource) && z.test(e)) throw this.Z('Document fields cannot begin and end with "__"');
    }
}
...
/** Parse document data from a set() call. */ function __PRIVATE_parseSetData(e, t, r, n, i, s = {}) {
    ...
    if (s.merge) u = new FieldMask(o.fieldMask), _ = o.fieldTransforms; else if (s.mergeFields) {
        ...
        for (const n of s.mergeFields) {
            ...
            if (!o.contains(i)) throw new FirestoreError(P.INVALID_ARGUMENT, `Field '${i}' is specified in your field mask but missing from your input data.`);
...
class __PRIVATE_ArrayUnionFieldValueImpl extends FieldValue {
    ...
    isEqual(e) {
        return e instanceof __PRIVATE_ArrayUnionFieldValueImpl && o(this.te, e.te);
    }
}
...
class __PRIVATE_ArrayRemoveFieldValueImpl extends FieldValue {
    ...
    isEqual(e) {
        return e instanceof __PRIVATE_ArrayRemoveFieldValueImpl && o(this.te, e.te);
    }
}
...
/** Parse update data from a list of field/value arguments. */ function __PRIVATE_parseUpdateVarargs(e, t, r, n, i, s) {
    const o = e.ue(1 /* UserDataSource.Update */ , t, r), a = [ __PRIVATE_fieldPathFromArgument(t, n, r) ], _ = [ i ];
    if (s.length % 2 != 0) throw new FirestoreError(P.INVALID_ARGUMENT, `Function ${t}() needs to be called with an even number of arguments that alternate between field names and values.`);
 ...
 */ function __PRIVATE_parseData(e, t) {
    ...
    // from firestore-exp.
    e = u(e))) return __PRIVATE_validatePlainObject("Unsupported field value:", t, e), 
 ...
 */ function __PRIVATE_fieldPathFromArgument(e, t, r) {
    ...
    // FieldPath.
    t = u(t)) instanceof FieldPath) return t._internalPath;
...
function __PRIVATE_createError(e, t, r, n, i) {
    ...
    u += ")"), new FirestoreError(P.INVALID_ARGUMENT, a + e + u);
}
...
function query(e, t, ...r) {
    ...
    t instanceof AppliableConstraint && n.push(t), n = n.concat(r), function __PRIVATE_validateQueryConstraintArray(e) {
        ...
        if (t > 1 || t > 0 && r > 0) throw new FirestoreError(P.INVALID_ARGUMENT, "InvalidQuery. When using composite filters, you cannot use more than one filter at the top level. Consider nesting the multiple filters within an `and(...)` statement. For example: change `query(query, where(...), or(...))` to `query(query, and(where(...), or(...)))`.");
    }
    /**
 * @license
 * Copyright 2020 Google LLC
 ...
 */ class QueryFieldFilterConstraint extends QueryConstraint {
    ...
    _parse(e) {
        const t = __PRIVATE_newUserDataReader(e.firestore), r = function __PRIVATE_newQueryFilter(e, t, r, n, i, s, o) {
            let a;
            if (i.isKeyField()) {
                if ("array-contains" /* Operator.ARRAY_CONTAINS */ === s || "array-contains-any" /* Operator.ARRAY_CONTAINS_ANY */ === s) throw new FirestoreError(P.INVALID_ARGUMENT, `Invalid Query. You can't perform '${s}' queries on documentId().`);
 ...
 */ class QueryOrderByConstraint extends QueryConstraint {
    ...
    _apply(e) {
        const t = function __PRIVATE_newQueryOrderBy(e, t, r) {
            if (null !== e.startAt) throw new FirestoreError(P.INVALID_ARGUMENT, "Invalid query. You must not call startAt() or startAfter() before calling orderBy().");
            if (null !== e.endAt) throw new FirestoreError(P.INVALID_ARGUMENT, "Invalid query. You must not call endAt() or endBefore() before calling orderBy().");
...
/** Helper function to create a bound from a document or fields */ function __PRIVATE_newQueryBoundFromDocOrFields(e, t, r, n) {
    if (r[0] = u(r[0]), r[0] instanceof DocumentSnapshot) return function __PRIVATE_newQueryBoundFromDocument(e, t, r, n, i) {
        if (!n) throw new FirestoreError(P.NOT_FOUND, `Can't use a DocumentSnapshot that doesn't exist for ${r}().`);
        const s = [];
        ...
        // results.
            ...
            if (__PRIVATE_isServerTimestamp(e)) throw new FirestoreError(P.INVALID_ARGUMENT, 'Invalid query. You are trying to start or end a query using a document for which the field "' + r.field + '" is an uncommitted server timestamp. (Since the value of this field is unknown, you cannot start/end a query with it.)');
            if (null === e) {
                const e = r.field.canonicalString();
                throw new FirestoreError(P.INVALID_ARGUMENT, `Invalid query. You are trying to start or end a query using a document for which the field '${e}' (used as the orderBy) does not exist.`);
            }
            s.push(e);
        }
 ...
 */ (e._query, e.firestore._databaseId, t, r[0]._document, n);
    {
        ...
        return function __PRIVATE_newQueryBoundFromFields(e, t, r, n, i, s) {
            ...
            if (i.length > o.length) throw new FirestoreError(P.INVALID_ARGUMENT, `Too many arguments provided to ${n}(). The number of arguments must be less than or equal to the number of orderBy() clauses`);
            const a = [];
            for (let s = 0; s < i.length; s++) {
                const u = i[s];
                if (o[s].field.isKeyField()) {
                    if ("string" != typeof u) throw new FirestoreError(P.INVALID_ARGUMENT, `Invalid query. Expected a string for document ID in ${n}(), but got a ${typeof u}`);
                    if (!__PRIVATE_isCollectionGroupQuery(e) && -1 !== u.indexOf("/")) throw new FirestoreError(P.INVALID_ARGUMENT, `Invalid query. When querying a collection and ordering by documentId(), the value passed to ${n}() must be a plain document ID, but '${u}' contains a slash.`);
                    const r = e.path.child(ResourcePath.fromString(u));
                    if (!DocumentKey.isDocumentKey(r)) throw new FirestoreError(P.INVALID_ARGUMENT, `Invalid query. When querying a collection group and ordering by documentId(), the value passed to ${n}() must result in a valid document path, but '${r}' is not because it contains an odd number of segments.`);
...
function __PRIVATE_parseDocumentIdValue(e, t, r) {
    if ("string" == typeof (r = u(r))) {
        if ("" === r) throw new FirestoreError(P.INVALID_ARGUMENT, "Invalid query. When querying with documentId(), you must provide a valid document ID, but it was an empty string.");
        if (!__PRIVATE_isCollectionGroupQuery(t) && -1 !== r.indexOf("/")) throw new FirestoreError(P.INVALID_ARGUMENT, `Invalid query. When querying a collection by documentId(), you must provide a plain document ID, but '${r}' contains a '/' character.`);
        const n = t.path.child(ResourcePath.fromString(r));
        if (!DocumentKey.isDocumentKey(n)) throw new FirestoreError(P.INVALID_ARGUMENT, `Invalid query. When querying a collection group by documentId(), the value provided must result in a valid document path, but '${n}' is not because it has an odd number of segments (${n.length}).`);
        return __PRIVATE_refValue(e, new DocumentKey(n));
    }
    if (r instanceof DocumentReference) return __PRIVATE_refValue(e, r._key);
    throw new FirestoreError(P.INVALID_ARGUMENT, `Invalid query. When querying with documentId(), you must provide a valid string or a DocumentReference, but it was: ${__PRIVATE_valueDescription(r)}.`);
}
 ...
 */ function __PRIVATE_validateDisjunctiveFilterElements(e, t) {
    if (!Array.isArray(e) || 0 === e.length) throw new FirestoreError(P.INVALID_ARGUMENT, `Invalid Query. A non-empty array is required for '${t.toString()}' filters.`);
}
 ...
 */ function __PRIVATE_validateNewFieldFilter(e, t) {
    ...
    throw r === t.op ? new FirestoreError(P.INVALID_ARGUMENT, `Invalid query. You cannot use more than one '${t.op.toString()}' filter.`) : new FirestoreError(P.INVALID_ARGUMENT, `Invalid query. You cannot use '${t.op.toString()}' filters with '${r.toString()}' filters.`);
}

function __PRIVATE_validateQueryFilterConstraint(e, t) {
    if (!(t instanceof QueryFieldFilterConstraint || t instanceof QueryCompositeFilterConstraint)) throw new FirestoreError(P.INVALID_ARGUMENT, `Function ${e}() requires AppliableConstraints created with a call to 'where(...)', 'or(...)', or 'and(...)'.`);
}
...
class AbstractUserDataWriter {
     ...
     */    convertVectorValue(e) {
        const t = e.fields?.[B].arrayValue?.values?.map((e => __PRIVATE_normalizeNumber(e.doubleValue)));
 ...
 */ function getDocs(e) {
    (function __PRIVATE_validateHasExplicitOrderByForLimitToLast(e) {
        if ("L" /* LimitType.Last */ === e.limitType && 0 === e.explicitOrderBy.length) throw new FirestoreError(P.UNIMPLEMENTED, "limitToLast() queries require specifying at least one orderBy() clause");
...
function updateDoc(e, t, r, ...n) {
    ...
    // performing validation.
        let s;
    s = "string" == typeof (t = u(t)) || t instanceof FieldPath ? __PRIVATE_parseUpdateVarargs(i, "updateDoc", e._key, t, r, n) : __PRIVATE_parseUpdateData(i, "updateDoc", e._key, t);
