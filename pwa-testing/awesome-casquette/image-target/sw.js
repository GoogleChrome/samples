...
self.addEventListener('fetch', event => {
  ...
  function respondToShare(event) {
    event.respondWith((async () => {
      ...
      const formData = await event.request.formData(); // (event.request.method === 'POST') ? await event.request.formData() : (new URL(event.request.url)).searchParams;
/*
...
*/
      ...
      const file_fields = ['received_html_files', 'received_css_files', 'received_image_files', 'received_jpeg_files', 'received_png_files', 'received_svg_files', 'received_webp_files'];
      ...
      async function progress() {
        ...
        if (file_fields[field_index] === 'received_image_files' || file_fields[field_index] === 'received_svg_files') {
          file_contents += '<img src="data:image/svg+xml;base64,' + btoa(dataFromFileLoaded) + '">'
        } else if (file_fields[field_index] === 'received_jpeg_files') {
          file_contents += '<img src="data:image/jpeg;base64,' + btoa(dataFromFileLoaded) + '">'
        } else if (file_fields[field_index] === 'received_png_files') {
          file_contents += '<img src="data:image/png;base64,' + btoa(dataFromFileLoaded) + '">'
        } else if (file_fields[field_index] === 'received_webp_files') {
          file_contents += '<img src="data:image/webp;base64,' + btoa(dataFromFileLoaded) + '">'
        } else {
  ...
  if (handleClientSide && pathname.endsWith('/share-target-destination.html')) {
